...
const findMatchingAudioFile = (audio: Express.Multer.File[], metadata: Express.Multer.File): Express.Multer.File | undefined => {
    const withoutExtension = metadata.originalname.replace(/\.[^.]*$/, '');
    return audio.find((file) => file.originalname.startsWith(withoutExtension));
}

export const uploadHandler = multer({ storage: storage }).fields([{ name: 'audio' }, { name: 'metadata' }]);
...
export default async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (method != 'POST') {
        ...
        try {
            ...
            const count = clipIds.reduce((total, value) => value != -1 ? total + 1 : total, 0);
            console.log(`${count} clips uploaded under ${newClientId} with batch label ${status}`);
